...
function renderSectionBody(doc, html, ctx) {
  ...
  while ((m = noteRe.exec(html))) {
    renderFlow(doc, parseFlow(html.slice(last, m.index)), { x: ctx.M, width: ctx.CW, size: 10, base: INK });
    last = noteRe.lastIndex;
    renderNote(doc, m[2], m[1], ctx);
  }
  renderFlow(doc, parseFlow(html.slice(last)), { x: ctx.M, width: ctx.CW, size: 10, base: INK });
}
