...
function releaseGroups() {
  ...
  for (const ln of db.pendingReleaseLines()) {
    ...
    byId.get(p.id).boxes.push({ line_id: ln.id, ym: pr ? pr.ym : null, release_at: ln.release_at, days: daysUntil(ln.release_at), box: boxLite(item) });
  }
  return byId;
}
...
function releaseSearch(q) {
  ...
  if (mode === 'box') {
    for (const g of groups) for (const b of g.boxes) (sat(b.release_at) ? matched : pending).push({ person: g.person, ...b });
...
function peopleForNotif(opts, refDate) {
  ...
  const sat = (rel) => criterion === 'exact' ? rel === refDate : rel <= refDate;
  const byId = new Map();
  for (const ln of db.pendingReleaseLines()) {
    const p = qrDb.getPerson(ln.person_id);
    if (!p) continue;
    ...
    byId.get(p.id).boxes.push({ line_id: ln.id, item, release_at: ln.release_at });
  }
  const people = [];
  for (const g of byId.values()) {
    const satisfying = g.boxes.filter(b => sat(b.release_at));
    const include = mode === 'all' ? (g.boxes.length > 0 && g.boxes.every(b => sat(b.release_at))) : satisfying.length > 0;
    if (!include) continue;
    people.push({
      person: g.person, boxes: g.boxes, satisfying,
      total: g.boxes.length, satisfiedCount: satisfying.length,
      allOut: g.boxes.every(b => sat(b.release_at)),
    });
  }
  people.sort(byName);
