...
class TestLasDosCifras(unittest.TestCase):
    ...
    def test_la_ponderada_es_0_86(self):
        self.assertAlmostEqual(
            long_isoform_fraction(TABLA.sites, weighted=True), 0.8558, places=4
        )

    def test_la_sin_ponderar_es_0_65(self):
        self.assertAlmostEqual(
            long_isoform_fraction(TABLA.sites, weighted=False), 0.6496, places=4
        )

    def test_la_de_trabajo_es_la_PONDERADA_y_lo_dice(self):
        self.assertAlmostEqual(TABLA.working_value, 0.8558, places=4)
        self.assertIn("condicionado a muestras", TABLA.why_weighted)
    ...
    def test_sin_ningun_sitio_distal_aborta(self):
        solo_proximales = tuple(s for s in TABLA.sites if not s.distal)
        with self.assertRaises(ValueError):
...
class TestLoQueNoSeDaPorBueno(unittest.TestCase):
    ...
    def test_no_queda_ninguna_comprobacion_BLOQUEANTE(self):
        self.assertEqual(TABLA.pending, ())
