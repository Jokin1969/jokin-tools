"""Tests de la descarga (paso 0). Regla 5: escritos antes que `shmir_design/fetch.py`.
...
from shmir_design.errors import FetchError
from shmir_design.fetch import build_efetch_url, parse_fasta_payload
...
class TestConstruccionDeLaPeticion(unittest.TestCase):
    ...
    def test_api_key_y_cortesia_se_anaden_si_se_dan(self):
        url = build_efetch_url(BASE, "NM_011170.3", api_key="K", tool="shmir-design")
        self.assertIn("api_key=K", url)
        self.assertIn("tool=shmir-design", url)
