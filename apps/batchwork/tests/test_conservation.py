...
Regla 5: escritos antes que `batchwork/conservation.py`.
...
from batchwork.filters import FilterState
from batchwork.conservation import (
    Utr3,
...
class TestVentanasDelBloque(unittest.TestCase):
    ...
    def test_la_ventana_del_offset_1_falla_solo_por_GC(self):
...
class TestInforme(unittest.TestCase):
    ...
    def test_el_informe_enseña_los_motivos_de_fallo(self):
        ...
        self.assertIn("NOT_RUN", texto)    # la asimetria, que no llego a correr
...
@unittest.skipUnless(
    ...
    "NOT_RUN: faltan los 3'UTR completos; obtenlos con tools/fetch_data.py "
    "--efetch-url <base verificada>; no se sustituyen por secuencia sintetica (regla 1)",
)
class TestUtrCompletos(unittest.TestCase):
    ...
    def utrs(self):
        ...
        return (
            Utr3("raton", read_fasta_sequence(MOUSE_FASTA)),
            Utr3("humano", read_fasta_sequence(HUMAN_FASTA)),
        )
