...
class Report:
    ...
    def format_text(self) -> str:
        ...
        for aviso in self.avisos:
            ...
            if aviso.affected:
                lines.append(f"     Ventanas afectadas: {', '.join(aviso.affected)}")
        ...
        avisos.append(
            Aviso(
                ...
                message=(
                    ...
                    f"Los {len(afectadas)} candidato(s) corriente abajo podrian no "
                    f"capturar la isoforma corta. NO se excluyen: quedan anotados con "
                    f"riesgo_APA=True y la decision es del responsable."
                ),
                affected=afectadas,
            )
        )
